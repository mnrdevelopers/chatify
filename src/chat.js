      ...
      snapshot.docChanges().forEach(change => {
        if (change.type === 'added') {
          ...
          if (myUid && data.uid !== myUid && !myBlockedUsers.includes(data.uid)) {
            ...
            // ── Background: show OS push notification ────────────────────────
            ...
            // ── Foreground: dispatch custom event for in-app toast ────────────
            // Fires whether hidden or visible so the main thread decides what to show.
            // Main thread suppresses the toast when this chat is already open.
            window.dispatchEvent(new CustomEvent('chatify:new-message', {
              detail: { chatId, senderName, text: textSnippet },
            }));
          }
        }
      });
