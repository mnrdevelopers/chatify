...
import { initOneSignal, stopOneSignal } from './onesignal.js';
import { 
...
function renderRecentChats(chats) {
  ...
  visibleChats.forEach(chat => {
    ...
    card.dataset.chatid = chat.id; // needed for toast click-to-open
...
function initApp() {
  ...
  async function requestNotificationPermission() {
    ...
    try {
      ...
      console.warn('[Notif] requestPermission() failed:', err);
      return 'denied';
    }
  }
  ...
  // ── In-App Notification Toast (foreground, different chat) ──────────────
  // Listens for chatify:new-message events dispatched by chat.js when a new
  // message arrives in ANY open Firestore listener. Shows a toast only when
  // the message is NOT from the currently active chat.
  window.addEventListener('chatify:new-message', (e) => {
    const { chatId: incomingChatId, senderName, text } = e.detail || {};
    // Suppress if user is already viewing that chat
    if (incomingChatId && incomingChatId === currentChatId) return;
    // Suppress if app is hidden (OS notification already shown by chat.js)
    if (document.visibilityState === 'hidden') return;
    showToast(`💬 ${senderName}: ${text}`, incomingChatId);
  });

  // ── Toast helper ───────────────────────────────────────────
  let _toastTimeout = null;
  function showToast(message, chatIdToOpen) {
    const toast = document.getElementById('toast-notification');
    if (!toast) return;
    toast.textContent = message;
    toast.classList.remove('hidden');
    toast.style.cursor = chatIdToOpen ? 'pointer' : '';
    toast.onclick = chatIdToOpen ? () => {
      toast.classList.add('hidden');
      // Find the chat in the recent list and open it
      const recentItem = document.querySelector(`.recent-chat-item[data-chatid="${chatIdToOpen}"]`);
      if (recentItem) recentItem.click();
    } : null;
    if (_toastTimeout) clearTimeout(_toastTimeout);
    _toastTimeout = setTimeout(() => toast.classList.add('hidden'), 5000);
  }
  ...
  (() => {
    ...
    if (params.get('incomingCall') === '1') {
      ...
      console.log('[FCM] Cold-start from call notification, chatId:', params.get('chatId'));
    }
  })();
      ...
      try {
        ...
        if (profile && profile.phoneNumber) {
          ...
          requestNotificationPermission().then(perm => {
            console.log('[Notif] Final permission state:', perm);
            if (perm === 'denied') {
              showToast('⚠️ Notifications are blocked. Enable them in browser settings.');
            }
          });

          // ── OneSignal: register device for push notifications ──────────────
          // Serverless — token saved to Firestore, no custom backend needed.
          initOneSignal(user.uid,
            (subId) => console.log('[OneSignal] ✅ Device registered, subscription saved.'),
            (err)   => console.warn('[OneSignal] Could not start push notifications:', err)
          );

          // Listen for foreground OneSignal notifications to deep-link into a chat
          window.addEventListener('onesignal:open-chat', (e) => {
            const chatId = e.detail?.chatId;
            if (chatId) {
              const chatItem = document.querySelector(`[data-chatid="${chatId}"]`);
              if (chatItem) chatItem.click();
            }
          });
          window.addEventListener('onesignal:incoming-call', (e) => {
            const chatId = e.detail?.chatId;
            if (chatId) {
              // Firestore listener in listenForIncomingCalls will surface the modal
              console.log('[OneSignal] Incoming call notification for chat:', chatId);
            }
          });
  ...
  btnLogout.addEventListener('click', async () => {
    ...
    await stopOneSignal(auth.currentUser?.uid); // Opt out of OneSignal on logout
    await logoutUser();
  });
