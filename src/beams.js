...
export async function initBeams(firebaseUid, onSuccess, onError) {
  ...
  // Check notification permission before attempting init
  if (Notification.permission === 'denied') {
    console.warn('[Beams] Notifications are blocked by user. Skipping Beams init.');
    return;
  }
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES + 1; attempt++) {
    try {
      ...
      console.error(`[Beams] ❌ Init failed (attempt ${attempt}/${MAX_RETRIES + 1}):`, err);
      beamsClient = null; // Reset so next attempt creates a fresh client

      if (attempt <= MAX_RETRIES) {
        // Exponential backoff: 1s, 2s
        const delay = attempt * 1000;
        console.warn(`[Beams] Retrying in ${delay}ms...`);
        await new Promise(resolve => setTimeout(resolve, delay));
      } else {
        // All retries exhausted
        window.dispatchEvent(new CustomEvent('chatify:beams-error', { detail: { err } }));
