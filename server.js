...
// Map: firebaseUid → last heartbeat timestamp (for stale-entry cleanup)
const onlineTimestamps = new Map();

// Clean up entries that haven't sent a heartbeat in 90 seconds
// (handles browser crashes, mobile backgrounding, network drops)
setInterval(() => {
  const now = Date.now();
  for (const [uid, ts] of onlineTimestamps.entries()) {
    if (now - ts > 90_000) {
      onlineUsers.delete(uid);
      onlineTimestamps.delete(uid);
      console.log(`[Socket] Removed stale entry for ${uid} (no heartbeat)`);
    }
  }
}, 30_000);
...
io.on('connection', (socket) => {
  ...
  socket.on('user-online', (uid) => {
    if (uid) {
      ...
      onlineTimestamps.set(uid, Date.now());
      console.log(`[Socket] ${uid} is online (${socket.id})`);
    }
  });

  // Periodic heartbeat so we can detect stale/crashed clients
